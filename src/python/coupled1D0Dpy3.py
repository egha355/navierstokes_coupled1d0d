...
# You can also write the list or dictionary in a file with the same name.var. In python2 dictionary can be indexed but in 
# python 3 it can't. Therefor we make it list first before using index. like list( vari.values() ) [i]
def workSpace(vari,variableType='All'):
  ...
  for i in range(len(vari)):
    value=list(vari.values())[i]
    if type(value)==list or type(value)==dict:
      print(list(vari.keys())[i],type(value),len(value),str(sys.getsizeof(value))+' Bytes')
  #==========================
  ...
  if variableType=='All':
    for i in range(len(vari)):
      ...
      if type(value) not in [list,dict]:
        print(list(vari.keys())[i],list(vari.values())[i])
  #==========================
